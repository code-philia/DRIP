...
from gcg.neuralexec import NeuralExecConfig, run
from config import (
    DELIMITERS,
    PROMPT_FORMAT,
...
if __name__ == "__main__":
    ...
    parser.add_argument('-a', '--attack', type=str, default='neuralexec',
                        choices=["neuralexec"],
                        nargs='+')
    ...
    llm_inputs = []
    for i, d in tqdm(enumerate(data[:4]), "Attacking samples..."):
        d_item = deepcopy(d)
        # gcg attack
        ...
        result = run(model=model,
                     tokenizer=tokenizer,
                     messages=orig_str,
                     enclosed_instruction=TEST_INJECTED_PROMPT.capitalize(),
                     target=TEST_INJECTED_WORD.lower(),
                     frontend_delimiter=frontend_delimiters,
                     config=config,
                     pass_expert_labels=args.pass_expert_labels)

        d_item["input"] = d_item["input"] + " " + result.best_string[0] + " " + TEST_INJECTED_PROMPT.capitalize() + " " + result.best_string[1]
