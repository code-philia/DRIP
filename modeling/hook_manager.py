...
from .llama_instsep import LlamaMLPMoE
from losses.cka import calculate_gram_batch
...
class HookManager:
    def __init__(self, model: nn.Module,
        ...
        if self.hook_fn is None:
            ...
            hook_fn_dict = {'flatten': self.flatten_hook_fn,
                            'avgpool': self.avgpool_hook_fn}
            if self.hook_fn in hook_fn_dict:
                self.hook_fn = hook_fn_dict[self.hook_fn]
            else:
                raise ValueError(f"No hook function named {self.hook_fn}. "
                                 f"Options: {list(hook_fn_dict.keys())}")
    ...
    def register_hooks(self, hook_fn: Callable) -> None:
        ...
        print(f"{new_num_handles - prev_num_handles} Hooks registered. "
              f"Total hooks: {new_num_handles}")
    ...
    def flatten_hook_fn(self, module: nn.Module, inp: torch.Tensor, out: torch.Tensor) -> None:
        ...
        if self.calculate_gram:
            feature = calculate_gram_batch(feature)
