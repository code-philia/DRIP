...
if __name__ == '__main__':
    ...
    '''Step 3: Orig Data, DPO, one preferred answer, one rejected answer'''
    ...
    for it, item in enumerate(data_list):
        ...
        preference_data.append({
            'instruction': d_item["instruction"],
            "input":    item["data_prompt_clean"],
            'output':   clean_response,
        })

    random.shuffle(preference_data)
    with open('./datasets/sep/sep_data_origdata_sft.json', 'w', encoding='utf-8') as f:
        json.dump(preference_data, f, ensure_ascii=False, indent=2)

    '''Orig data, DPO training'''
    with open('./datasets/sep/sep_data_dpo.json', 'r', encoding='utf-8') as f:
        data_list = json.load(f)

    outputs = []
    for item in data_list:
        instruction = item["prompt"].split("\n\n[MARK] [INPT][COLN]\n")[0].split("[MARK] [INST][COLN]\n")[1]
        input = item["prompt"].split("\n\n[MARK] [INPT][COLN]\n")[1].split("\n\n[MARK] [RESP][COLN]")[0]
        chosen = item["chosen"].split("</s>")[0]
        rejected = item["rejected"].split("</s>")[0]
    ...
    with open('./datasets/sep/sep_data_origdata_dpo.json', 'w', encoding='utf-8') as f:
